...
class WOFHumanPlayer(WOFPlayer):
    def getMove(category,pbscuredPhrase,guessed):
        print("""
        {name} has ${prizeMoney}
        ...
        Guess a letter, phrase, or type 'exit' or 'pass':""".format(name=self.name,prizeMoney=self.prizeMoney,category=self.category,obscured_phrase=self.obscured_phrase,guessed=self.guessed))
        answer=input()
        return answer
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        chosen_move=random.randint(1,10)
        if chosen_move>self.difficulty:
            return True
        else:
            return False
    def getPossibleLetters(self,guessed):
        ...
        for i in LETTERS:
            if i in guessed:
                ...
                if self.prizeMoney<250 and i in VOWELS:
                    continue
                else:
                    lst.append(i)
        return lst
